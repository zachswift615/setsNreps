...
export default class HomePage extends Component {
  ...
  deleteSession = () => {
    fetch("http://localhost:8000/api/session/${session_id}", {
      ...
      .then(this.props.refreshSessionList);
  };
    ...
    return (
      <div className="container home-page-container">
        <div className={"top-bar"}>
          <button onClick={this.logout} className={"btn btn-sm btn-info"}>
            logout
          </button>
        </div>
        <h1>Start Workout</h1>
        <button className="btn btn-block btn-secondary" onClick={this.onClick}>
          Create New Workout
        </button>
